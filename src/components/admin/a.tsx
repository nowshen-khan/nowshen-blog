...
export default function SiteSettingsForm() {
  ...
  const handleNavbarLinkChange = (index: number, field: 'label' | 'href', value: string) => {
            ...
            {formData.navbar?.links?.map((link: any, idx: number) => (
              <div key={idx} className="flex gap-2 items-center">
                <Input
                  placeholder="Label"
                  value={link.label}
                  onChange={e => handleNavbarLinkChange(idx, 'label', e.target.value)}
                />
                <Input
                  placeholder="Href"
                  value={link.href}
                  onChange={e => handleNavbarLinkChange(idx, 'href', e.target.value)}
                />
                <Button variant="destructive" size="sm" onClick={() => removeNavbarLink(idx)}>Remove</Button>
              </div>
            ))}
            <Button size="sm" onClick={addNavbarLink}>Add Link</Button>
          </div>
        </CardContent>
      </Card>
          ...
          <div className="space-y-2">
            ...
            {formData.footer?.links?.map((link: any, idx: number) => (
              <div key={idx} className="flex gap-2 items-center">
                ...
                />
                <Button variant="destructive" size="sm" onClick={() => removeFooterLink(idx)}>Remove</Button>
              </div>
            ))}
            <Button size="sm" onClick={addFooterLink}>Add Link</Button>
          </div>
            ...
            {['facebook','twitter','github','linkedin','email'].map((key) => (
              <Input
                ...
                onChange={e => setFormData({
                  ...
                  footer: {
                    ...formData.footer,
                    socialLinks: { ...formData.footer?.socialLinks, [key]: e.target.value }
                  }
                })}
              />
            ))}
